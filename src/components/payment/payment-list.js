...
import {StringUtils, MathUtils, Helper} from 'djmodules-utils';

class PaymentList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            amountPrice: props.sumPrice,
            paymentList: JSON.parse(JSON.stringify(props.paymentList))
        }
    }
    ...
    ok = () => {
        if (this.state.amountPrice > 0) {
            message.destroy();
            message.warning('金额未结清');
        } else {
            let usePayment = _.filter(this.state.paymentList || [], item => {
                return Number(item.amount) > 0;
            });
            this.props.ok(JSON.parse(JSON.stringify(usePayment)));
        }
    }


    render() {
        let {sumPrice, paymentMode, mutexPaymentList} = this.props;
        let {paymentList, amountPrice} = this.state;
        /**支付方式模式：single-单一支付方式,multiple-多支付方式 */
        /**获取正在使用的有效支付方式*/
        ...
        >
            <div className="payment-list">
                <div className="title">
                    <div>还需付款：<em className="important">¥{StringUtils.numberFormat(amountPrice, 2)}</em></div>
                    <div>共需支付：<em>¥{StringUtils.numberFormat(sumPrice, 2)}</em></div>
                </div>
