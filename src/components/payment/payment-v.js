...
class PaymentV extends (PureComponent || Component) {
    ...
    componentDidMount() {
        ...
        let paymentByYunPOS = _.filter(validPaymentList || [], item => {
            return (item.payMethod === 13 || item.payMethod === 14 || item.payMethod === 15) && item.cashierDeviceInfo && item.cashierDeviceInfo.device === 2;
        }).length > 0;
    ...
    render() {
        ...
        >
            <div style={{margin: 30, textAlign: 'center'}}>
                ...
                <div style={{fontSize: '18px', marginTop: 10}}>支付中...请勿关闭浏览器</div>
                {tryAgainElement}
            </div>
        </Modal>)
    }
