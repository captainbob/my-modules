...
class BatchAddModal extends Component {
    ...
    _handleOk = () => {
        ...
        HttpUtil.promiseAjax("/rs/goodsx/sspu/check_mcode", {
            method: 'POST',
            data: {
                merchantCodes: textValue
            }
        }).then(res => {
            let successList = res.filter(v => v.success),
                failureList = res.filter(v => !v.success);
            let successIds = successList.map(v => v.brandSpuId);
            ...
            <div style={style}>
                ...
                >
                    <Row type="flex" justify="space-between" align="top" gutter={16}>
                        ...
                        <Col span={14}>
                            <Alert message="温馨提示" type="info" description="打开excel，选择货号这一栏，整列复制，然后粘贴到左侧框中" />
                            {
